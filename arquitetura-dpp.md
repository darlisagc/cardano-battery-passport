...
- **Opcao A** e ideal se voce quer **total independencia**: seus dados ficam
  ...
  e maior. **Atencao LGPD/GDPR:** dados gravados on-chain nao podem ser apagados
...
| `relatorio_emissao_html.py` | Relatorio | Recibo HTML local pos-emissao (todos os atores), gerado a partir do payload — sem verificacao on-chain. Link Cexplorer da tx. |
| `relatorio_reciclagem_html.py` | Relatorio | Relatorio HTML local adicional (so `--ator reciclagem`), gerado a partir do payload — sem verificacao on-chain. Materiais recuperados, rastreabilidade reversa e fluxo de ciclo de vida. |
...
| **Smart contract** | Programa que roda na blockchain e valida dados automaticamente antes de permitir o registro. Como um funcionario-robo que confere formularios. No Cardano, chamados de **validators**. |
