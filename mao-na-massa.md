...
| **Verificação** | unificada — qualquer combinacao A+B+C usa `verificador.py` (Seção 3\) | mesmo | `https://app.preprod.uverify.io/verify/<data_hash>/<index>?serial=<serial>` |
...
2. Click na opção “Create”  
3. Apos isto, selecione “Write Text” and type DPP.  
...
- **Metadata nativa** — payloads pequenos, dados que precisam ser lidos por **qualquer parte sem dependência** (auditoria pública, regulador, integração custom, ambientes air-gapped).  
