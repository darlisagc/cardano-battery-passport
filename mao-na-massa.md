...
**Caminho 2 — API do UVerify** (fallback). Se o Caminho 1 não encontrou `uverify_template_id` na metadata nativa, a transação foi provavelmente emitida via UVerify (SDK ou UI). Nesse caso, o payload DPP completo **não está na blockchain** — o smart contract Plutus do UVerify grava apenas um *anchor hash* (a impressão digital `sha256(gtin+serial)`) no script datum on-chain, enquanto o payload rico fica armazenado off-chain no servidor UVerify, indexado por esse `data_hash`.

O desafio é: o verificador precisa descobrir qual `data_hash` usar para consultar a API. Para isso, reúne candidatos de **três fontes** (em ordem de confiabilidade):

1. **Hint da cadeia** (`ref_*_data_hash`) — a credencial que referencia esta tx já carrega o `data_hash` do ator referenciado como campo do payload. É o caminho mais rápido e confiável: um lookup direto sem precisar inspecionar a blockchain. Funciona sempre que os payloads foram gerados pelo `_payloads.py` (que inclui os hints automaticamente).
2. **Redeemer on-chain** — quando o smart contract Plutus do UVerify processa uma emissão, o redeemer da transação contém a estrutura `UVerifyStateRedeemer`, que inclui a lista de certificados emitidos. O verificador navega essa estrutura via Blockfrost (`redeemer.fields[1].list[*].fields[0].bytes`) para extrair o `data_hash` real. É o caminho mais confiável quando não há hint disponível (ex: credencial emitida pela UI sem campos `ref_*_data_hash`).
3. **Inline datum** (heurístico) — último recurso: o verificador varre o inline datum dos outputs da transação procurando qualquer sequência de exatamente 32 bytes (tamanho de um SHA-256). Pode gerar falsos positivos (ex: o ID do State Datum também tem 32 bytes), por isso cada candidato é testado contra a API do UVerify antes de ser aceito.

Para cada candidato, o verificador faz um **HTTP GET direto** na API pública do UVerify (`GET /api/v1/verify/{data_hash}`) em vez de usar o método `verify_by_transaction()` do SDK Python. O motivo é técnico: a resposta JSON da API inclui o campo `stateDatum` com o histórico completo de atualizações do smart contract, que pode ter **centenas de níveis de aninhamento**. O decodificador JSON do CPython atinge o limite de recursão ao parsear essa estrutura, causando um `RecursionError`. A chamada HTTP direta contorna isso lendo apenas os campos necessários (`metadata`, `transactionHash`) sem precisar parsear a resposta inteira.
